...
class ConfigInit(LanguageSelection):
    ...
    def __init__(self):
        ...
        config.plugins.tmdb.debug_log_level = ConfigSelection(
            default="INFO", choices=list(log_levels.keys()))
        config.plugins.tmdb.cover_size = ConfigSelection(
            default="original", choices=["w92", "w185", "w500", "original"])
        config.plugins.tmdb.backdrop_size = ConfigSelection(
            default="original", choices=["w300", "w780", "w1280", "original"])
        config.plugins.tmdb.lang = ConfigSelection(
        ...
        config.plugins.tmdb.trailer_player = ConfigSelection(
