...
class ScreenMain(Picture, Json, Screen, HelpableScreen):
    ...
    def getData(self, menu_selection, text, ident, page, callback):
        ...
        reactor.callFromThread(callback, totalpages,
                               result)  # pylint: disable=E1101
        ...
        if self.menu_selection:
            if result:
                self["searchinfo"].setText(
                    "%s (%s %s/%s)" % (self.search_title, _("page"), self.page, totalpages))
            else:
                self['searchinfo'].setText(
                    _("No results for: %s") % self.search_title)
        else:
            if result:
                if text != self.text:
    ...
    def moveUp(self):
        logger.info("SelectionIndex: %s, len(self.result): %s",
                    self["list"].getSelectionIndex(), len(self.result))
