    ...
    def getResult(self, res, menu_selection, text, ident, page):
        ...
        for entry in results["results"]:
            ...
            if media == "movie" and title_movie:
                title = "%s (%s, %s)" % (
                    title_movie, _("Movie"), date_movie[:4])
                search_title = title_movie
            elif media == "tv" and title_series:
                title = "%s (%s, %s)" % (
                    title_series, _("Series"), date_tv[:4])
            ...
            if media == "person":
                cover_url = "http://image.tmdb.org/t/p/%s%s" % (
                    config.plugins.tmdb.cover_size.value, profile_path)
            else:
                cover_url = "http://image.tmdb.org/t/p/%s%s" % (
                    config.plugins.tmdb.cover_size.value, cover_path)
            backdrop_url = "http://image.tmdb.org/t/p/%s%s" % (
                config.plugins.tmdb.backdrop_size.value, backdrop_path)
